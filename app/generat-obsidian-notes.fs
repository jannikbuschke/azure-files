  ...
  interface IRequestHandler<GenerateObsidianNotes, ApiResult<MediatR.Unit>> with
    member this.Handle(_, _) =
      taskResult {
        ...
        let entities = entities |> Seq.filter(fun v-> not (v.Tags |> List.contains "mark-for-cleanup"))
...
tags:
- type/photo
{v.Tags|>List.map(fun v->"- photo/"+v)|>String.concat "\n"}
{properties}
---
...
"""
        ...
        for (file,md) in markdown do
          do! System.IO.File.WriteAllLinesAsync($"C:\\home\\jannik\\obsidian\\default\\photos\\photo - {file.Id.value().ToString().ToLower()}.md", [|md|])
