...
module NodaTime =
    ...
    let parseDateTimeStringToInstant v =
        ...
        if strContainsOnlyNumber v then
            v |> Int64.Parse |> NodaTime.Instant.FromUnixTimeMilliseconds |> Some
        else
            match
            ...
            with
            | true, value -> value |> NodaTime.Instant.FromDateTimeOffset |> Some
            | false, _ -> None


    let asLocalDate (t: NodaTime.Instant) =
        t.ToDateTimeUtc() |> NodaTime.LocalDate.FromDateTime
...
type ImageVariant =
    { Name: string
      Url: string
      PublicUrl: Skippable<string option>
      Dimension: Dimension }
...
type FileProjection =
    ...
    member this.FileDateOrCreatedAt =
        this.DateTimeOriginal |> Option.defaultValue this.CreatedAt
    ...
    member this.Apply(e: FileEvent, m: Marten.Events.IEvent) : FileProjection =
        ...
        let applyPropertyChanged fileProjection e =
            ...
            | PropertyChanged.PropertyRemoved propertyName ->
                { fileProjection with
                    Properties = (fileProjection.Properties |> List.filter (fun v -> v.Name <> propertyName)) }
            | PropertyChanged.PropertyUpdated property ->
                { fileProjection with
                    Properties =
                        (fileProjection.Properties
                         |> List.map (fun v -> if v.Name = property.Name then property else v)) }
        ...
        | PropertiesChanged propertyChanges ->
            propertyChanges |> List.fold (fun acc v -> applyPropertyChanged acc v) this
        | PropertyChanged e -> applyPropertyChanged this e
        | PublicUrlCreated e ->
            match e.Variant with
            | Root -> { this with PublicUrl = Include(Some e.Url) }
            | VariantName name ->
                { this with
                    LowresVersions =
                        this.LowresVersions
                        |> List.map (fun v ->
                            if v.Name = name then
                                { v with PublicUrl = Include(Some e.Url) }
                            else
                                v) }
...
type FileInfo =
    { Extension: Extension; Type: FileType }
...
type FileViewmodel =
    ...
    member this.DateTimeAsLocalDate() =
        ...
        |> Option.map (fun date -> date.ToDateTimeUtc() |> NodaTime.LocalDate.FromDateTime)
    ...
    static member FromFileProjection(file: FileProjection) =
        ...
        let fileDateOrCreated = dateTimeOriginal |> Option.defaultValue file.CreatedAt
        ...
        let lat =
            latitude
            |> Option.map (fun v -> v |> Array.map (fun v -> (decimal v.Numerator) / (decimal v.Denominator)))
        ...
        task {
            let where, parameters =
                ...
                | RangeFilter(from, until) ->
                    "data ->> 'OriginalDateTime' <= @from", [ ("@from", Sql.string from); "@until", Sql.string until ]
